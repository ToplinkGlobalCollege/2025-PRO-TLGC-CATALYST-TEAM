# TLGC CATALYST TEAM
This engineering documentation presents the technical details and design specifications of the autonomous vehicle robot developed by the TLGC Catalyst Future Engineer's Team for the Philippine Robotics Olympiad 2025

# Content
**control**- This folder contains the file to control motors, and sensors on the EV3.

**docs**- This folder contains documentation like the robot’s architecture, programming flow, and assembly instructions. Helps others understand our EV3 robot.

**hardware**- This folder contains the EV3-specific hardware files, chassis designs, and part configurations.

**images**- This folder contains photos of our EV3 robot, design diagrams, wiring diagrams.

**obstacle**-This folder contains the obstacle management strategy used by the robot to navigate the challenge course. It outlines how the robot detects, decides, and maneuvers around obstacles to maintain smooth and uninterrupted movement throughout the competition.

**software**- This folder contains the code used to control the EV3 robot.

**team**- This folder contains info about the team members, their roles, and the team photos

**video**- This folder contains our robot's run which is published on YouTube.

# Credits
